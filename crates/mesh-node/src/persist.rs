...
/// Sequential-storage map key for the standalone settings record (13-byte
/// `LSET` encoding from the `settings` module). Independent of the node
/// record: settings writes never touch `PERSIST_VERSION` or node bytes,
/// and old node images keep decoding without migration.
pub const KEY_SETTINGS: u8 = 0x02;
/// Sequential-storage map key for the standalone WiFi credential record
/// (102-byte `WSET` encoding owned by `firmware/src/wifi.rs`). Independent
/// of the node record: credential writes never touch `PERSIST_VERSION` or
/// node bytes. The passphrase is stored in plaintext flash; anyone with
/// physical USB/flash access can dump it — documented, not a leak path.
pub const KEY_WIFI: u8 = 0x03;
...
mod layout_tests {
    use super::*;
    /// Append-only contract: header offsets and contact-0 bytes are frozen.
    /// Any field insert/shift fails here before it can corrupt persisted state.
    #[test]
    fn header_and_contact_zero_offsets_are_frozen() {
        let contacts = [PersistContact::empty(); MAX_CONTACTS];
        let view = PersistView::new(
            true,
            7,
            [9u8; 8],
            true,
        ...
        let (bytes, n) = encode_persist(&view);
        assert_eq!(n, PERSIST_LEN);
        // Header: magic | version | flags | label | uid | uid_set | unix | tx_epoch | sign_set | sign.
        assert_eq!(&bytes[0..4], b"LMSH");
        assert_eq!(bytes[4], PERSIST_VERSION);
        assert_eq!(bytes[5], 0x01);
        assert_eq!(bytes[6], 7);
        assert_eq!(&bytes[7..15], &[9u8; 8]);
        assert_eq!(bytes[15], 1);
        assert_eq!(&bytes[16..24], &0x0102_0304_0506_0708u64.to_be_bytes());
        assert_eq!(&bytes[24..28], &0xAABBCCDDu32.to_be_bytes());
        assert_eq!(bytes[28], 1);
        assert_eq!(&bytes[29..61], &[5u8; 32]);
        // Contact slot length and stride: 213 bytes per the frozen formula.
        assert_eq!(CONTACT_BYTES, 213);
        let base = 61usize;
        assert_eq!(PERSIST_LEN, base + MAX_CONTACTS * CONTACT_BYTES);
        assert_eq!(PERSIST_V2_LEN, base + 2 * CONTACT_BYTES);
        // Empty slot: present/blocked zero, windows empty (e=1, h=b=0).
        assert_eq!(bytes[base], 0);
        assert_eq!(bytes[base + 1], 0);
        let win0 = base + 1 + 1 + 32 + 8 + 32 + 1 + 8 + 8 + 4 + 4 + 12;
        assert_eq!(bytes[win0 + 16], 1);
        assert_eq!(bytes[win0 + 16 + 17 * 2 + 17], 1);
    }
}
