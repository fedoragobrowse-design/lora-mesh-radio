...
use mesh_node::persist::{
    decode_view, migrate_to_v3, KEY_SETTINGS, KEY_WIFI, PERSIST_LEN, PERSIST_V2_LEN,
};
...
pub enum StorageReq {
    ...
    /// Durably commit one 13-byte `LSET` settings record under the
    /// separate `KEY_SETTINGS` map key. Never touches node bytes or
    /// `PERSIST_VERSION`.
    CommitSettings { bytes: [u8; 13] },
    /// Durably commit one 102-byte `WSET` WiFi credential record under the
    /// separate `KEY_WIFI` map key. Passphrase stored in plaintext flash
    /// (physical-access readable; documented, never in logs/replies).
    CommitWifi { bytes: [u8; 102] },
}
...
pub enum StorageResp {
    ...
    /// Stored 13-byte `LSET` settings record (clamped on decode; caller
    /// falls back to defaults when absent).
    SettingsLoaded {
        bytes: [u8; 13],
    },
    /// Stored 102-byte `WSET` WiFi credential record (validated on decode;
    /// absent/corrupt means unconfigured).
    WifiLoaded {
        bytes: [u8; 102],
    },
        ...
        loop {
            match self.rx.receive().await {
                StorageReq::Load => {
                    ...
                    // Settings ride alongside: absent key means defaults.
                    match map
                        .fetch_item::<heapless::Vec<u8, 13>>(&mut buf, &KEY_SETTINGS)
                        .await
                    {
                        Ok(Some(v)) => {
                            let mut bytes = [0u8; 13];
                            let n = core::cmp::min(v.len(), 13);
                            bytes[..n].copy_from_slice(&v[..n]);
                            let _ = self.tx.send(StorageResp::SettingsLoaded { bytes }).await;
                        }
                        Ok(None) | Err(_) => {
                            let _ = self
                                .tx
                                .send(StorageResp::SettingsLoaded { bytes: [0u8; 13] })
                                .await;
                        }
                    }
                    // WiFi credential rides alongside: absent key means unconfigured.
                    match map
                        .fetch_item::<heapless::Vec<u8, 102>>(&mut buf, &KEY_WIFI)
                        .await
                    {
                        Ok(Some(v)) => {
                            let mut bytes = [0u8; 102];
                            let n = core::cmp::min(v.len(), 102);
                            bytes[..n].copy_from_slice(&v[..n]);
                            let _ = self.tx.send(StorageResp::WifiLoaded { bytes }).await;
                        }
                        Ok(None) | Err(_) => {
                            let _ = self
                                .tx
                                .send(StorageResp::WifiLoaded { bytes: [0u8; 102] })
                                .await;
                        }
                    }
                }
                ...
                StorageReq::CommitSettings { bytes } => {
                    let mut v: heapless::Vec<u8, 13> = heapless::Vec::new();
                    let _ = v.extend_from_slice(&bytes);
                    match map.store_item(&mut buf, &KEY_SETTINGS, &v).await {
                        Ok(()) => {
                            let _ = self.tx.send(StorageResp::Committed(Ok(()))).await;
                        }
                        Err(_) => {
                ...
                StorageReq::CommitWifi { bytes } => {
                    let mut v: heapless::Vec<u8, 102> = heapless::Vec::new();
                    let _ = v.extend_from_slice(&bytes);
                    match map.store_item(&mut buf, &KEY_WIFI, &v).await {
                        Ok(()) => {
                            let _ = self.tx.send(StorageResp::Committed(Ok(()))).await;
                        }
                        Err(_) => {
