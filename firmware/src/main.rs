...
async fn runtime_task(
    ...
    let usb_state0 = usb::UsbState::new(engine, health);
...
async fn main(spawner: Spawner) {
    ...
    loop {
        match STORAGE_RESP.receive().await {
            ...
            StorageResp::Committed(_) => continue,
        }
    }
    ...
    #[cfg(feature = "radio")]
    {
        // Runtime owns engine/TRNG/radio deadlines; USB directions progress
        // independently so a reply or received event needs no further input.
        spawner.spawn(runtime_task(engine, health, radio_version, hw_present, trng).unwrap());
    ...
    #[cfg(not(feature = "radio"))]
    {
        embassy_futures::join::join(device.run(), async {
            ...
            usb_state.radio_version = radio_version;
            let mut lines = usb::LineBuffer::new();
