...
No port arguments. USB serials are logged for identity only -- the live
`status` label (never a serial table or ACM numbering) decides A/B/C.
Asserts: all three present, radio-secure, SX1276
...
duplicates, no wrong recipients; post-send drains cover the 12 s ACK
retry window so late retries are seen) and TX attempts within the
send+retry+relay bound (up to 3 attempts per send + forwarded relays +
1 ACK); empty slot 3 on
...
raw non-UTF-8 bytes fail `BAD_REQUEST` against id 0 with no airtime,
probed over the held session without closing it (a close would drop the
port lock and buffered events); blocked contact fails
`CONTACT_BLOCKED` with zero airtime then unblocks clean; 5-message burst
ACKs each with exact-once (C, contact 2) delivery, no
duplicates/wrong recipients (arrival order reported, never a FAIL gate:
relay/retries may reorder); parser still healthy after hostile inputs.
