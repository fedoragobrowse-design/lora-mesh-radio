...
SEND_TIMEOUT = 60.0
# Post-send collective drain: must cover the one 12 s ACK wait plus relay
# jitter per leg so late duplicates landing after an early leg still fail it.
DRAIN_WINDOW = 15.0
MAX_CONTACTS = 16
...
def parse_args(argv=None):
    ...
    p.add_argument(
        "--drain",
        type=float,
        default=DRAIN_WINDOW,
        help="post-send collective drain seconds; must cover the retry budget "
        "(scales +5s per edge when left at default)",
    )
...
def main(argv=None) -> int:
    ...
    drain_window = max(1.0, min(float(args.drain), 120.0))
    if args.drain == DRAIN_WINDOW:
        drain_window = max(drain_window, 5.0 * len(resolve_edges(args.edge)))
    ...
    try:
        ...
        for label, s in nodes.items():
            reply, events = call(s, "time_set", {"unix_seconds": now}, timeout=15.0)
            for e in events:
                if isinstance(e, dict) and e.get("event") == "received":
                    check(f"{label} unexpected traffic on time sync", False, str(e)[:120])
            check(f"{label} time sync", reply is not None and reply.get("ok"), str(reply)[:120])
        for label, s in nodes.items():
            reply, events = call(s, "radio_set", {"enabled": True}, timeout=15.0)
            for e in events:
                if isinstance(e, dict) and e.get("event") == "received":
