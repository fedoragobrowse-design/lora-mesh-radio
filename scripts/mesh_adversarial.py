...
SEND_TIMEOUT = 45.0
# Post-send collective drain: must exceed one 12 s ACK wait plus relay
# jitter so late retries/duplicates still land inside the window.
DRAIN_WINDOW = 15.0
# Post-burst collective drain: last burst ACK ends its retries, but relayed
# duplicates of earlier burst frames can still be in flight; allow margin.
BURST_DRAIN_WINDOW = 30.0
...
def attempts(s: SerialSession) -> int | None:
    ...
    if r is None:
        return None
    counters = r.get("result", {}).get("counters", {})
    if "tx_attempts" not in counters:
        check("tx_attempts present", False, str(counters)[:120])
        return None
    return counters["tx_attempts"]


def expect_reject(s, label, op, params, err, tag):
    n0 = attempts(s)
    if n0 is None:
        check(f"{label} {tag} airtime readable", False, "no status")
        return
    ...
    n1 = attempts(s)
    if n1 is None:
        check(f"{label} {tag} airtime readable after", False, "no status")
        return
    check(f"{label} {tag} no airtime", n1 == n0, f"n0={n0} n1={n1}")
...
def expect_send(tx, txl, tx_cid, text, rx, rxl, rx_cid, tag, nodes):
    ...
    info = {c.get("contact_id"): c for c in raw_slots if isinstance(c, dict)}.get(tx_cid, {})
...
def main() -> int:
    ...
    try:
        ...
        # BAD_REQUEST with no airtime (engine validate_body rejects). Uses
        # the held B session's serial directly: closing the session would
        # release the port lock and reset TX ids, letting another process
        # grab the port mid-probe and losing buffered events.
        import json as _json
        n0 = attempts(B)
        if n0 is None:
            check("B airtime readable before raw probe", False, "no status")
            return 1
        bser = B._ser
        B._pending = b""
        try:
            bser.reset_input_buffer()
        except Exception:
            pass
        try:
            bser.write(b'{"id":700,"op":"send","contact_id":2,"text":"bad\xff\xfe"}\n')
            bser.flush()
        except Exception as exc:
            check("B raw probe write", False, f"{type(exc).__name__}: {exc}")
            return 1
        got = None
        t0 = time.monotonic()
        bser.timeout = 6
        while time.monotonic() - t0 < 8:
            try:
                ...
                check("B raw probe read", False, f"{type(exc).__name__}: {exc}")
                break
            if not line:
                continue
            try:
                ...
                continue
            if isinstance(d, dict) and d.get("id") in (700, 0):
                got = d
                break
        B._pending = b""
        ...
        n1 = attempts(B)
        check("B invalid-utf8 no airtime", n1 is not None and n1 == n0, f"n0={n0} n1={n1}")
        deadline = time.monotonic() + 5.0
        while time.monotonic() < deadline:
            try:
                evt = C.read_next(max(0.1, deadline - time.monotonic()))
            except Exception as exc:
                check("C drain after raw probe", False, f"{type(exc).__name__}: {exc}")
                break
            if evt is None:
                continue
            if isinstance(evt, dict) and evt.get("event") == "received":
                check("C no delivery from invalid utf8", False, str(evt)[:120])
        ...
        deadline = time.monotonic() + BURST_DRAIN_WINDOW
        while time.monotonic() < deadline:
            for board_label, s in nodes.items():
                remaining = deadline - time.monotonic()
                if remaining <= 0:
        ...
        # Exact-once per marker is the FAIL gate; order is reported only:
        # a relayed/retried frame may legitimately reorder, so an order
        # mismatch must never fail an otherwise intact burst.
        order = sorted((m for m in markers if m in c_first_seen), key=lambda m: c_first_seen[m])
        check("C burst intact", burst_ok, f"hits={ {m: burst_hits[m] for m in markers} }")
        if burst_ok and order != markers:
            print(f"NOTE C burst order differs (relay/retry reorder): {order}", flush=True)
